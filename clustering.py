...
#         self.df = pd.read_csv("/Users/pavandeekshith/B-Tech/Btech_3rd_Year/6th_Sem/Geospatial_clustering/ML_Assignment_Dataset_mod.csv")  # Specify delimiter
...
#         silhouette_scores = {
#             "K-Means": silhouette_score(self.df[['latitude', 'longitude']], self.df['kmeans']),
#             "DBSCAN": silhouette_score(self.df[['latitude', 'longitude']], self.df['dbscan']) if len(set(self.df['dbscan'])) > 1 else -1,
#             "Agglomerative": silhouette_score(self.df[['latitude', 'longitude']], self.df['agglo']),
#             "GMM": silhouette_score(self.df[['latitude', 'longitude']], self.df['gmm'])
#         }
        
#         davies_bouldin_scores = {
#             "K-Means": davies_bouldin_score(self.df[['latitude', 'longitude']], self.df['kmeans']),
#             "DBSCAN": davies_bouldin_score(self.df[['latitude', 'longitude']], self.df['dbscan']) if len(set(self.df['dbscan'])) > 1 else -1,
#             "Agglomerative": davies_bouldin_score(self.df[['latitude', 'longitude']], self.df['agglo']),
#             "GMM": davies_bouldin_score(self.df[['latitude', 'longitude']], self.df['gmm'])
#         }
...
#         algorithms = [(self.df['kmeans'], "K-Means"), (self.df['dbscan'], "DBSCAN"),
#                       (self.df['agglo'], "Agglomerative"), (self.df['gmm'], "GMM")]
        
#         for ax, (labels, title) in zip(axes.flatten(), algorithms):
#             scatter = ax.scatter(self.df['longitude'], self.df['latitude'], c=labels, cmap='viridis', alpha=0.5)
...
#         """Creates a folium map with clustered points."""
...
# # Usage Example
# file_path = "/Users/pavandeekshith/B-Tech/Btech_3rd_Year/6th_Sem/Geospatial_clustering/ML_Assignment_Dataset_mod.csv"  # Replace with actual file path
...
# silhouette_scores, davies_bouldin_scores = clustering.evaluate_clusters()
...
#         and normalizes latitude and longitude using StandardScaler.
...
#     def evaluate_clusters(self):
#         """
#         Computes clustering evaluation metrics (Silhouette Score & Davies-Bouldin Score).
        
#         Returns:
#             tuple: A dictionary of silhouette scores and Davies-Bouldin scores for each algorithm.
#         """
#         silhouette_scores = {}
#         davies_bouldin_scores = {}
